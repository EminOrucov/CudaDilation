...
__global__ void dilatacion(unsigned char *d_output, unsigned char *d_input, int width, int height, int SIZE_STRUCTURING_ELEMENT)
{
	...
	*/
	...
	int row = i;
	int col = j;
	for (int x = -SIZE_STRUCTURING_ELEMENT; x <= SIZE_STRUCTURING_ELEMENT; x++) {
		for (int y = -SIZE_STRUCTURING_ELEMENT; y <= SIZE_STRUCTURING_ELEMENT; y++) {
			int neighborRow = row + x + SIZE_STRUCTURING_ELEMENT;
			int neighborCol = col + y + SIZE_STRUCTURING_ELEMENT;
				
				int idx = neighborRow * width + neighborCol;
				if (d_input[idx] > cur) {
					cur = d_input[idx];
				}
		}
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	dim3 grid(((HEIGHT + 2 * SIZE_STRUCTURING_ELEMENT) + (block.y - 1)) / block.y, ((WIDTH + 2 * SIZE_STRUCTURING_ELEMENT) + (block.x - 1)) / block.x);
	// TO DO - Ejecutar el kernel
	dilatacion<< <grid, block >> > (d_output, d_input, WIDTH, HEIGHT, SIZE_STRUCTURING_ELEMENT);
  ...
  for (y = 0; y < HEIGHT; y++) {
    for (x = 0; x < WIDTH; x++) {
		if (host_output[(y + SIZE_STRUCTURING_ELEMENT)*WIDTH + x + SIZE_STRUCTURING_ELEMENT] != gpu_output[(y + SIZE_STRUCTURING_ELEMENT)*WIDTH + x + SIZE_STRUCTURING_ELEMENT]) {
        errors++;
